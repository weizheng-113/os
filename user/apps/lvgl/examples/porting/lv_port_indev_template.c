...
static void mouse_read(lv_indev_t *indev_drv, lv_indev_data_t *data)
{
    if (use_window)
    {
        /*Get the current x and y from window manager*/
        char buf[64];
        sprintf(buf, "windowmanager get_xy_pressed %d", wid);
        uint64_t ret = send_window_msg(window_input_index, window_output_index, (const char *)buf);
        if ((int64_t)ret < 0)
            abort();

        uint16_t x = ((ret >> 0) & 0xFFFF);
        uint16_t y = ((ret >> 16) & 0xFFFF);
        bool pressed = (((ret >> 32) & 0xFFFFFFFF) != 0);
        if (((ret >> 32) & 0xFFFFFFFF) == 0x7FFFFFFF)
        {
            return;
        }
