...
int hba_bind_sbuf(struct hba_cmdh *cmdh, struct hba_cmdt *cmdt, void *buf, uint32_t len)
{
    if (len > 0x400000UL)
    {
        kinfo("AHCI buffer too large");
        return -1;
    }

    uint64_t buf_phys = translate_addr(get_kernel_page_dir(), (uint64_t)buf);
    if (buf_phys == 0)
    {
        kinfo("AHCI buffer not mapped");
        return -1;
    }
...
int ahci_init_device(struct hba_port *port)
{
    ...
    free_frames(translate_addr(get_kernel_page_dir(), (uint64_t)cmd_table), 1);
    return 1;
...
uint64_t ahci_read(void *data, uint64_t lba, void *buffer, uint64_t size)
{
    ...
    spin_lock(&ahci_op_lock);
    dev->ops.submit(dev, &req);
    spin_unlock(&ahci_op_lock);
    return size;
}
...
struct ahci_driver *ahci_driver_init(pci_bar_base_address *bar5)
{
    ...
    page_map_range_to(get_kernel_page_dir(), (uint64_t)hba->base, bar5->address, bar5->size, KERNEL_PTE_FLAGS);
    if (hba->base == NULL)
    {
        kinfo("ahci driver init failed");
        free(ahci_drv);
        return NULL;
    }
    ...
    for (uint64_t i = 0, fisp = 0, clbp = 0; i < 32; i++, pmap >>= 1, fisp = (fisp + 1) % 16, clbp = (clbp + 1) % 4)
    {
        ...
        kinfo("sata%d: %s, blk_size=%d, blk=0..%d",
              i,
              hbadev->model,
              hbadev->block_size,
              hbadev->max_lba);
...
uint64_t ahci_init()
{
    ...
    {
        kinfo("ahci device has no bar5");
        return -1;
    }

    spin_init(&ahci_op_lock);

    op_buffer = (void *)alloc_frames(0x400000UL / 0x1000UL);
